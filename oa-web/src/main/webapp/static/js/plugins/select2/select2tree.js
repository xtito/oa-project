(function($) {
	...
	function moveOption(id) {
		...
		if(id) {
            var $select2Results = $(".select2-results__options").find("li[parent=" + id + "]");
			$select2Results.insertAfter(".select2-results__options li[val=" + id + "]");
			$select2Results.each(function() {
				moveOption($(this).attr("val"));
			});
		} else {
            var $select2not = $(".select2-results__options").find("li:not([parent])");
			$select2not.appendTo(".select2-results__options ul");
			$select2not.each(function() {
				moveOption($(this).attr("val"));
			});
		}
	}
	...
	function switchAction(id, open) {
        var $select2 = $(".select2-results__options").find("li[parent='" + id + "']");
